...
接收一批待翻译的条目 (JSON 数组)，对每条**翻译** `incoming` (英文) 为中文，输出填好 `review` 字段的 JSON 数组。

> **绝大多数条目需要翻译。**只有极少数纯代码/纯 URL 变更才跳过。

> **输出要求**：仅输出一个有效的 JSON 数组，不要输出任何其他文字或 markdown 标记。每个元素必须包含 `file`、`lines`、`current`、`incoming`、`review` 字段。
