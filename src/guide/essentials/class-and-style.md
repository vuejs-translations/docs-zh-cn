...
数据绑定的一个常见需求场景是操纵元素的 CSS 类列表和内联样式。因为它们都是 attribute，我们可以使用 `v-bind` 来做这件事：我们只需要通过表达式计算出一个字符串作为最终结果即可。然而频繁地连接字符串让人很闹心，也很容易出错的。因此，Vue 专为 `class` 和 `style` 的 `v-bind` 用法提供了特殊的功能增强。除了字符串外，表达式的结果还可以是对象或数组。
...
对于只有一个根元素的组件，当你使用了 `class` attribute，这些类会被添加到根元素上，并且会合并该元素上已有的类。
...
如果你的组件有多个根元素，你将需要指定哪个根元素来接收这个类。你可以使用组件的 `$attrs` 这个属性实现指定：
