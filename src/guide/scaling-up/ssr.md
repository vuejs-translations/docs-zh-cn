...
3. 通过在 HTML 外壳中添加 [Import Map](https://html.spec.whatwg.org/multipage/webappapis.html#import-maps) 以支持在浏览器中使用 `import * from 'vue'`。
...
完整的实现会非常复杂，并且取决于你选择使用的构建工具链。因此，如果你需要 SSR，我们强烈建议使用 [Vue 框架](/guide/quick-start#frameworks)，因为它们通常内置了 SSR 支持。
