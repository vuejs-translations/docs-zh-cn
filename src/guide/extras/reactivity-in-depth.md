...
- `A0` 和 `A1` 被视为这个副作用的**依赖**，因为它们的值被用来执行这个副作用。因此这次副作用也可以说是一个它依赖的**订阅者**。

我们需要一个魔法函数，能够在 `A0` 或 `A1` (这两个**依赖**) 变化时调用 `update()` (产生**副作用**)。
