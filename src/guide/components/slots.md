...
在下面的示例中，我们定义了一个卡片组件，它拥有两个条件插槽：`header` 和 `footer`。
当 header 或 footer 存在时，我们希望包装它们以提供额外的样式：
