...
通过和下面 JavaScript 的函数作对比，来以另一种方式理解插槽：
...
不是只有 text 元素可用作插槽内容。插槽内容还可以是任意合法的模板内容，例如我们可以传入多个元素，甚至是组件：
...
上面的 `<FancyList>` 案例同时封装了可重用的逻辑 (数据获取、分页等) 和视图输出，但也将部分视图的最终输出通过作用域插槽交给了消费者组件来管理。
...
虽然这个模式很有趣，但大部分能用使用无渲染组件实现的功能都可以通过组合式 API 以另一种更高效的方式实现，且不会带来额外组件嵌套的开销。之后我们会在[可组合函数](/guide/reusability/composables.html)一章中介绍如何更高效地实现追踪鼠标位置的功能。
