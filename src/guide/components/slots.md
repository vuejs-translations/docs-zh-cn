...
![展示插槽内容从父组件注入到子组件插槽出口的图示](./images/slots.png)
...
![展示布局组件中多个具名插槽的图示，父组件的内容被分别导向对应的 header、main 和 footer 插槽](./images/named-slots.png)
...
我们也确实有办法这么做！可以像对组件传递 props 那样，向一个插槽的出口上传递 attributes。父级模板通过 `v-slot` 接收插槽 props，而子级模板会在渲染时将 props 传递给插槽出口：
...
注意 `v-slot="receivedProps"` 与插槽函数的签名相匹配。和函数参数类似，我们也可以在 `v-slot` 中使用解构：
