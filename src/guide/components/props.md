...
### 合并绑定时的合并行为 {#merge-behavior-when-combining-bindings}
...
- **事件监听器** —— 当在 `v-bind` 对象中传入监听器时，[使用 `onEventName` 键的约定](/guide/extras/render-function#v-on)。同一事件的所有处理函数都会被调用 (参见 [`v-on` 监听器继承](/guide/components/attrs#v-on-listener-inheritance))：
...
- **`class` 和 `style`** 遵循类似的合并策略 (参见 [`class` 与 `style` 的合并](/guide/components/attrs#class-and-style-merging))。

:::tip
完整的合并规则参见 [`mergeProps()`](/api/render-function#mergeprops) API 参考。
:::
