...
当组件*提供*一个值时，该组件的所有后代组件都可以选择使用 `inject` 来获取该值。与 prop 不同，提供组件不知道哪些组件正在接收该值。
...
传递给插槽的 prop 只能在父模板的特定区域中使用，该区域负责定义插槽的内容。该模板区域的行为类似于 prop 的变量作用域，因此称为“作用域插槽”。
...
插槽用于向子组件传递内容。prop 用于传递数据值，而插槽用于传递更丰富的内容，包括 HTML 元素和其他 Vue 组件。
