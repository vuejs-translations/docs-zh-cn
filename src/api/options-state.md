...
- **详细信息**
  ...
  该函数预期返回一个普通 JavaScript 对象，Vue 将使其成为响应式对象。实例创建后，可以通过 `this.$data` 访问原始数据对象。组件实例也代理了该数据对象上所有的属性，因此 `this.a` 等价于 `this.$data.a`。

  根数据对象一旦被侦听后，你就无法在其上面添加响应式属性。因此推荐在创建实例之前，预先声明所有的根级响应式属性。

  以 `_` 或 `$` 开头的属性将**不会**被组件实例代理，因为它们可能和 Vue 的内置属性、API 方法冲突。你必须以 `this.$data._property` 的方式访问它们。

  **不**推荐返回一个已具有其自身状态行为的对象，如浏览器 API 对象和原型属性等。理想状态下，返回的对象应是一个仅代表组件状态的普通对象。
...
- **详细信息**
  ...
  - 使用字符串数组的简易形式。
  - 使用对象的完整形式。该对象的每个属性键是 prop 的名称，且值是 prop 的类型 (一个构造函数) 或高级选项。
...
- **详细信息**
  ...
  除了一个根级属性，键也可以是一个简单的由点分隔的路径，例如 `a.b.c`。注意，这种用法**不支持**复杂表达式——仅支持由点分隔的路径。如果你需要侦听复杂的数据源，可以使用命令式的 [`$watch()`](/api/component-instance.html#watch) API。
...
声明当组件实例被父组件通过模板 refs 访问时暴露的公共属性。
...
- **详细信息**
  ...
  默认情况下，当通过 `$parent`、`$root` 或模板 refs 访问时，组件实例将向父组件暴露所有的实例属性。这可能不是我们希望看到的，因为组件很可能拥有一些应保持私有的内部状态或方法，以避免紧耦合。

  `expose` 选项期望一个属性名称字符串的列表。当使用 `expose` 时，只有显式列出的属性将在组件实例上暴露。
