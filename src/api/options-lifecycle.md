...
- **详细信息**
  ...
  **错误捕获注意事项**
  ...
  - 在拥有异步 `setup()` 函数 (带有顶层 `await`) 的组件中，即使 `setup()` 抛出了错误，Vue **也总是会**尝试渲染组件模板。这可能会导致更多的错误，因为在渲染组件模板的过程中，可能会尝试访问失败的 `setup()` 上下文中不存在的属性。当捕获此类组件中的错误时，请注意同时处理来自失败的异步 `setup()` (它们总是首先出现) 和失败的渲染过程的错误。
