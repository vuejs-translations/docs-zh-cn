...
接受一个内部值，返回一个响应式的、可更改的 ref 对象，此对象只有一个属性 `.value` 指向其内部的值。
...
- **详细信息**
  ...
  响应式转换是“深层”的：它会影响到所有嵌套的属性。一个响应式对象也可以深层地解包任何为 [ref](#ref) 属性，同时保持响应性。
...
- **详细信息**
  ...
  一个只读的代理是深层生效的，对任何嵌套属性的访问都将是只读的。它的 ref 解包行为与 `reactive()` 相同，但解包得到的值是只读的。
