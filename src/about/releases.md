...
小版本和大版本发布通常会经历一系列预发布阶段：**alpha**、**beta** 和**候选发布版 (RC)**。预发布版本的数量和类型取决于变更的范围。例如，变更有限的小版本可能只有 beta 阶段，而大版本通常会包含所有三个阶段，以便进行充分的测试并收集社区反馈。

你可以通过 `npx install-vue@alpha`、`npx install-vue@beta` 或 `npx install-vue@rc` 从 npm 安装最新的预发布版本。若要测试尚未包含在已标记的预发布版本中的更改，[vuejs/core](https://github.com/vuejs/core) 仓库的每一次提交都会发布为一个临时的持续发布预览版，你可以通过 `npx install-vue@edge` 安装它。
